...
install_git() {
  sudo make prefix="$GIT_INSTALL_DIR" all >$STDOUT_REDIR 2>&1 || return 1
  sudo make prefix="$GIT_INSTALL_DIR" install >$STDOUT_REDIR 2>&1 || return 1
}
...
if command -v git >/dev/null 2>&1 && [ "$(git --version || git -v)" == "git version $GIT_VERSION" ]; then
